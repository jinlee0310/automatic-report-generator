    ...
    try:
        while True:
            ...
            try:
                ...
                if data_type == "숫자":
                    get_number_query(column)
                elif data_type == "날짜":
                    print("형식을 입력해주세요.")
                    form = input()
                    get_date_query(column, form)
                elif data_type == "금액":
                    get_price_query(column)
                elif data_type == "율":
                    get_percent_query(column)
                elif data_type == "전화번호" or data_type == "전화 번호":
                    get_phone_number_query(column)
                elif data_type == "사업자번호":
                    get_business_number_query(column)
                elif data_type == "이메일":
                    get_email_query(column)
                elif data_type == "여부":
                    get_whether_query(column)
                else:
